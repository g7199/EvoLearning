...
Input layout (produced by scripts/run_experiment.py):
    ...
    reports/DATASET_Lk_seedS/METHOD/report.json   (bundled in this folder)
...
1.  Overrides DLELP / KnowLP with the graph-aware re-evaluation stored at
    dlelp_knowlp_graph_aware_results.json (bundled here). The original report.json
...
# No third-party dependencies: the mean and population std below are computed in pure Python, so this script runs on a clean Python 3 with nothing installed.

HERE = Path(__file__).resolve().parent
EXP_ROOT = HERE / 'reports'
OVERRIDE_PATH = HERE / 'dlelp_knowlp_graph_aware_results.json'
...
def aggregate(method, dataset, L, override, metric):
    ...
    var = sum(pow(x - mean, 2) for x in vals) / len(vals)
    std = pow(var, 0.5)
    return float(mean), float(std), len(vals), seeds_used

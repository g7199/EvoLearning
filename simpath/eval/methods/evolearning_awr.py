"""EvoLearning-AWR — Evo expert generation → AWR init → PPO fine-tune."""
import os
import torch
...
@register_method
class EvoLearningAWRMethod(BaseMethod):
    name = "EvoLearning-AWR"
    needs_experts = True
    needs_training = True
    ...
    def train(self, train_data, val_data, kes, graph, experts,
        ...
        # ═══ Stage 1: AWR initialization (Peng et al. 2019) ═══
        print(f"  [{self.name}] AWR on {len(experts)} experts...", flush=True)
        awr_s, awr_a, awr_ret = [], [], []
        for mastery, tgts, path, ep, *_ in experts:
            for step, action in enumerate(path[:L]):
                awr_s.append(make_state_standard(mastery, tgts, step, L, NC))
                awr_a.append(action)
                awr_ret.append(gamma ** (L - 1 - step) * ep)

        awr_s_t = torch.tensor(np.array(awr_s), dtype=torch.float32, device=dev)
        awr_a_t = torch.tensor(awr_a, dtype=torch.long, device=dev)
        awr_ret_t = torch.tensor(awr_ret, dtype=torch.float32, device=dev)

        awr_opt = torch.optim.Adam(policy.parameters(), lr=1e-3, weight_decay=1e-4)
        beta_awr = 1.0; weight_clip = 20.0

        awr_epochs = 2000
        total_steps = awr_epochs + n_episodes
        best_awr = -999; best_awr_state = None

        N_awr = len(awr_s); AWR_BATCH = 4096
        for epoch in range(awr_epochs):
            idx = torch.randperm(N_awr, device=dev)
            for i in range(0, N_awr, AWR_BATCH):
                bi = idx[i:i + AWR_BATCH]
                logits, vals = policy(awr_s_t[bi])
                adv = awr_ret_t[bi] - vals.detach()
                adv = (adv - adv.mean()) / (adv.std() + 1e-5)
                weights = torch.exp(adv / beta_awr).clamp(max=weight_clip)
                log_probs = F.log_softmax(logits, dim=-1)
                action_lp = log_probs.gather(1, awr_a_t[bi].unsqueeze(1)).squeeze(1)
                actor_loss = -(weights * action_lp).mean()
                critic_loss = F.mse_loss(vals, awr_ret_t[bi])
                loss = actor_loss + 0.5 * critic_loss
                awr_opt.zero_grad(); loss.backward()
                nn.utils.clip_grad_norm_(policy.parameters(), 1.0); awr_opt.step()

            if (epoch + 1) % 5 == 0:
                self._update_progress(out_dir, epoch+1, total_steps, reward=actor_loss.item())
            if (epoch + 1) % 200 == 0:
                policy.eval()
                eps = kes.evaluate_policy_batch(val_data, policy, L)
                vep = np.mean(eps); mk = ''
                if vep > best_awr:
                    best_awr = vep; best_awr_state = {k: v.clone() for k, v in policy.state_dict().items()}; mk = ' ***'
                print(f"    AWR Ep {epoch+1}/{awr_epochs} | Val={vep:+.4f}{mk}", flush=True)
                self._update_progress(out_dir, epoch+1, total_steps, val=vep)
                policy.train()

        if best_awr_state:
            policy.load_state_dict(best_awr_state)
        print(f"  [{self.name}] AWR Best Val = {best_awr:+.4f}", flush=True)
        ...
        # ═══ Stage 2: PPO fine-tune (standard, no DAPG) ═══
        ...
        for ep_i in range(n_episodes):
            ...
            if (ep_i + 1) % 5 == 0:
                self._update_progress(out_dir, awr_epochs + ep_i+1, total_steps, reward=all_rew[-1].mean())
            if (ep_i + 1) % val_interval == 0:
                ...
                self._update_progress(out_dir, awr_epochs + ep_i+1, total_steps, val=vep)
                if out_dir:

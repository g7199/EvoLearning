#!/usr/bin/env python3
"""
Train pykt DKT on ASSIST09 — pyKT standard config.
emb=200, dropout=0.1, lr=1e-3, batch=256.
...
print(f"{'='*60}")
print(f"Training pykt DKT on ASSIST09 (pyKT standard config)")
...
ckpt_path = 'outputs/checkpoints/pykt_dkt_best_assist09.pt'

# If checkpoint exists with old config, we'll overwrite
# Build skill mapping
kc_list = data['kc_list']
# Build kc_to_c: need to create a clean mapping
skill_map = {}
idx = 0
seen = set()
...
# Sanity check: mastery distribution
print(f"\n{'='*60}")
print("Mastery distribution check")
print(f"{'='*60}")
np.random.seed(42)
...
for s in sample:
    hc, hr = [], []
    for h in (s['train'] + s['rec_input'])[-MAX_LEN:]:
        for kc in h['kc_ids']:
            if kc in kc_to_c:
                hc.append(kc_to_c[kc]); hr.append(h['correct']); break
    if not hc: continue
    q = torch.tensor([hc], dtype=torch.long, device=DEVICE)
    r = torch.tensor([hr], dtype=torch.long, device=DEVICE)
    with torch.no_grad():
        m = dkt(q, r)[0, -1, :].cpu().numpy()
    all_m.append(m)
all_m = np.array(all_m)
print(f"  mean={all_m.mean():.3f}, std={all_m.std():.3f}")
print(f"  > 0.9: {(all_m > 0.9).mean()*100:.1f}%")
print(f"  > 0.95: {(all_m > 0.95).mean()*100:.1f}%")
print(f"  > 0.99: {(all_m > 0.99).mean()*100:.1f}%")
print(f"  range: [{all_m.min():.3f}, {all_m.max():.3f}]")

#!/usr/bin/env python3
"""Train DKVMN (Dynamic Key-Value Memory Network) on ASSIST15 dataSim split.
Cross-architecture cross-simulator: DKT (RNN) -> DKVMN (memory network)."""
...
DEV = args.device
MAX_LEN = 200
DIM_S = 200; SIZE_M = 50; DROPOUT = 0.1; LR = 1e-3; BATCH = 128; EPOCHS = 100; PATIENCE = 10

print(f"Training DKVMN on ASSIST15 (dim_s={DIM_S}, size_m={SIZE_M})")

with open('data/processed/assistments/assistments_processed.pkl', 'rb') as f:
    data = pickle.load(f)

# Skill mapping (same as DKT)
...
# CSEAL split (seed 42, identical to DKT training)
...
model = init_model('dkvmn', {'dim_s': DIM_S, 'size_m': SIZE_M, 'dropout': DROPOUT},
...
for ep in range(EPOCHS):
    ...
    for i in range(0, len(tr), BATCH):
        ...
        for j, (c, r) in enumerate(batch):
            ...
            m_b[j, :Li] = 1.0
        out = model(c_b, r_b)  # [B, T]
        # DKVMN output: per-step prediction for the question at that position
        # Train: predict r_b given q sequence and prior responses
        pred = out[:, :-1]
        target = r_b[:, 1:].float()
        mask = m_b[:, 1:]
...
torch.save({'model': model.state_dict(), 'num_c': NUM_C, 'skill_map': skill_map,
            'best_auc': best_auc, 'dim_s': DIM_S, 'size_m': SIZE_M, 'dropout': DROPOUT,
            'dataset': 'assist15', 'arch': 'dkvmn'},
           'outputs/checkpoints/dkvmn_assist15.pt')
